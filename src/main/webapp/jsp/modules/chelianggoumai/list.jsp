											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="cheliangbianhaoTh" class="sorting" onclick="sort('cheliangbianhao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">车辆编号</th>
																																																			...
																																																			<th id="dinggoushuliangTh" class="sorting" onclick="sort('dinggoushuliang')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">订购数量</th>
																																																			...
																																																			<th id="dinggoushijianTh" class="sorting" onclick="sort('dinggoushijian')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">订购时间</th>
	...
	<script >
		...
		function setDataRow(item){
			...
			btnGroup.setAttribute("class","form-button-action");
				...
				crossBtn.innerHTML = "收费"
				crossBtn.setAttribute("type","button")
				crossBtn.setAttribute("class","btn btn-secondary btn-xs");
				crossBtn.setAttribute("data-original-title","收费");
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"yunshushoufei\")");
			...
			function shDialog(id){
				$("#shId").attr("value",id);
				http("chelianggoumai/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
			...
			function sh(){
				...
				httpJson("chelianggoumai/update","POST",data,(res)=>{
					if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("chelianggoumai/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function getSum(colName){
			http("chelianggoumai" + colName,"GET",{
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"chelianggoumai");
        ...
        function pay(id){
            window.sessionStorage.setItem('id',id);
			window.sessionStorage.setItem('tableName',"chelianggoumai");
