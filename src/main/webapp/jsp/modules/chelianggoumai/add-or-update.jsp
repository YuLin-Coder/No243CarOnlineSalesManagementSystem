				...
				<div class="page-inner">
					<div class="page-header">
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="${pageContext.request.contextPath}/jsp/modules/chelianggoumai/list.jsp">车辆购买</a>
							</li>
						</ul>
					</div>
					<div class="row">
									...
									<div class="row">
										...
										<form id="addOrUpdateForm" method="post">
											...
											<div class="form-group">
												...
												<input id="cheliangtianchuang" name="cheliangtianchuang" class="form-control" placeholder="车辆天窗">
												</div>	
											<div class="form-group">
												<label>汽车排量</label>
											...
											<div class="form-group">
												...
												<input id="chezuo" name="chezuo" class="form-control" placeholder="车座">
												</div>	
											<div class="form-group">
												<label>车辆价格</label>
												
												<input id="cheliangjiage" name="cheliangjiage" class="form-control" onchange="calculation()">
												</div>	
											<div class="form-group">
												<label>订购数量</label>
												
												<input id="dinggoushuliang" name="dinggoushuliang" class="form-control" onchange="calculation()">
												</div>	
											<div class="form-group">
												<label>实付金额</label>
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "chelianggoumai/info/" + id,
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("chelianggoumai/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function calculation(){
			...
			var y = 0;
			var flag = 0;
																																																																																																																																																																																																									if($("#cheliangjiage").val() != null && $("#cheliangjiage").val() != ""){
								if(x == 0 && flag < 2){
									x = (x+1) * parseFloat($("#cheliangjiage").val());
									flag++;
								}else{
									x = x * parseFloat($("#cheliangjiage").val());
									flag++;
								}
							}
																								if($("#dinggoushuliang").val() != null && $("#dinggoushuliang").val() != ""){
								if(x == 0 && flag < 2){
									x = (x+1) * parseFloat($("#dinggoushuliang").val());
									flag++;
								}else{
									x = x * parseFloat($("#dinggoushuliang").val());
									flag++;
								}
							}
																																																																																																																			if(flag < 2){
		...
		function dataBind(){
			$("#updateId").val(ruleForm.id);	
												$("#cheliangbianhao").val(ruleForm.cheliangbianhao);	
																...
																$("#cheliangtianchuang").val(ruleForm.cheliangtianchuang);	
																$("#qichepailiang").val(ruleForm.qichepailiang);	
																$("#chezuo").val(ruleForm.chezuo);	
					...
					if(res.code == 0){
						...
						$('#updateId').val(crossTableId);
												...
												if(res.data != null && res.data != '' && res.data.cheliangtianchuang != null && res.data.cheliangtianchuang != ''){
															...
															$("#cheliangtianchuang").val(res.data.cheliangtianchuang);
													}
