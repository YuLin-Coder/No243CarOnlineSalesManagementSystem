...
<body>
	...
	<script >
		...
		$.ajax({ 
			type: "GET",
			url: baseUrl+"value/chelianggoumai/"+xColumnName+'/'+yColumnName,
			data:{ },
		...
		function group(colName){
			$.ajax({ 
                type: "GET",
                url: baseUrl+"group/chelianggoumai/"+colName,
             	data:{	tableName: "chelianggoumai",
                    	columnName: colName
					 },
                beforeSend: function(xhr) {
                    xhr.setRequestHeader("token", window.sessionStorage.getItem('token'));
