...
<body>
									...
									<div id="add-row_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																						<label>车辆颜色 <input type="text" id="cheliangyanseSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
	...
	<script >
		...
		function search(){
			searchForm = { key: ""};
																	if($('#cheliangyanseSearch').val() != null && $('#cheliangyanseSearch').val() != ''){
						searchForm.cheliangyanse ="%" +  $('#cheliangyanseSearch').val() + "%";
					}				
															getDataList();
		}
		// 获取数据列表
        function getDataList() {
			http("qicheyanse/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
																						cheliangyanse : searchForm.cheliangyanse,	
																		},(res)=>{
				if(res.code == 0){
					clear();
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("qicheyanse/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"qicheyanse");
