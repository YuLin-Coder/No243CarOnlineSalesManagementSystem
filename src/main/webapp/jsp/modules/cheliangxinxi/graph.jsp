...
<body>
	...
	<script >
		...
		$.ajax({ 
			type: "GET",
			url: baseUrl+"value/cheliangxinxi/"+xColumnName+'/'+yColumnName,
			data:{ },
		...
		function group(colName){
			$.ajax({ 
                type: "GET",
                url: baseUrl+"group/cheliangxinxi/"+colName,
             	data:{	tableName: "cheliangxinxi",
                    	columnName: colName
					 },
                beforeSend: function(xhr) {
                    xhr.setRequestHeader("token", window.sessionStorage.getItem('token'));
