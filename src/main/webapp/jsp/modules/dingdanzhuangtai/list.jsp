...
<body>
									...
									<div id="add-row_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																						<label>账号 <input type="text" id="zhanghaoSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																	...
																																																																																	<label>车辆品牌 <input type="text" id="cheliangpinpaiSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																	<label>车辆类型 <input type="text" id="cheliangleixingSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																																																																																																																																														<label>订单状态</label>
																<select name="dingdanzhuangtai" id="dingdanzhuangtaiSelect">
																																	<option value=" "> </option>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="zhanghaoTh" class="sorting" onclick="sort('zhanghao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">账号</th>
																																																			...
																																																			<th id="shouhuodizhiTh" class="sorting" onclick="sort('shouhuodizhi')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">收货地址</th>
																																																			...
																																																			<th id="yunshufeiyongTh" class="sorting" onclick="sort('yunshufeiyong')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">运输费用</th>
																																																			<th id="yujisongdaTh" class="sorting" onclick="sort('yujisongda')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">预计送达</th>
																																																			<th id="dingdanzhuangtaiTh" class="sorting" onclick="sort('dingdanzhuangtai')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">订单状态</th>
	...
	<script >
		...
		var tableName = "dingdanzhuangtai";
		var pageType = "list";
	  	var searchForm = { key: ""};
		var pageIndex = 1;
		var pageSize = 10;
		...
		function search(){
			searchForm = { key: ""};
						...
						searchForm.cheliangpinpai ="%" +  $('#cheliangpinpaiSearch').val() + "%";
																										...
																										if($('#cheliangleixingSearch').val() != null && $('#cheliangleixingSearch').val() != ''){
						searchForm.cheliangleixing ="%" +  $('#cheliangleixingSearch').val() + "%";
					}				
																																																													var dingdanzhuangtaiIndex = document.getElementById("dingdanzhuangtaiSelect").selectedIndex;
					var dingdanzhuangtaiOptions = document.getElementById("dingdanzhuangtaiSelect").options;
					var dingdanzhuangtaiSelectedOption = dingdanzhuangtaiOptions[dingdanzhuangtaiIndex]
					if(dingdanzhuangtaiSelectedOption.text != null &&  dingdanzhuangtaiSelectedOption.text != ''){
						searchForm.dingdanzhuangtai = "%" + dingdanzhuangtaiSelectedOption.text + "%";
					}		
															getDataList();
		}
		// 获取数据列表
        function getDataList() {
			http("dingdanzhuangtai/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
																						zhanghao : searchForm.zhanghao,	
																																	...
																																	cheliangleixing : searchForm.cheliangleixing,	
																																																																														dingdanzhuangtai : searchForm.dingdanzhuangtai,	
																		},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
            				...
            				var yunshufeiyongCell  = document.createElement('td');

												 yunshufeiyongCell.innerHTML = item.yunshufeiyong;

                
													row.appendChild(yunshufeiyongCell);
				
            				var yujisongdaCell  = document.createElement('td');

												 yujisongdaCell.innerHTML = item.yujisongda;

                
													row.appendChild(yujisongdaCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("dingdanzhuangtai/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function getSum(colName){
			http("dingdanzhuangtai" + colName,"GET",{
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"dingdanzhuangtai");
