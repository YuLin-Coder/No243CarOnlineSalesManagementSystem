...
<body>
	...
	<script >
		...
		$.ajax({ 
			type: "GET",
			url: baseUrl+"value/yunshushoufei/"+xColumnName+'/'+yColumnName,
			data:{ },
		...
		function group(colName){
			$.ajax({ 
                type: "GET",
                url: baseUrl+"group/yunshushoufei/"+colName,
             	data:{	tableName: "yunshushoufei",
                    	columnName: colName
					 },
                beforeSend: function(xhr) {
                    xhr.setRequestHeader("token", window.sessionStorage.getItem('token'));
