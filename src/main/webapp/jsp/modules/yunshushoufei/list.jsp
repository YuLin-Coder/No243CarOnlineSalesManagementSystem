	...
	<script >
		...
		function setDataRow(item){
			...
			btnGroup.setAttribute("class","form-button-action");
				...
				crossBtn.innerHTML = "状态"
				crossBtn.setAttribute("type","button")
				crossBtn.setAttribute("class","btn btn-secondary btn-xs");
				crossBtn.setAttribute("data-original-title","状态");
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"dingdanzhuangtai\")");
				if(crossBtnControl('状态') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("yunshushoufei/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"yunshushoufei");
        ...
        function pay(id){
            window.sessionStorage.setItem('id',id);
			window.sessionStorage.setItem('tableName',"yunshushoufei");
